...
from ftd.algorithms.modules import NormalizeImg, Flatten
from ftd.algorithms.modules import weight_init
...
class ImageAttentionSelectorLayers(nn.Module):
    ...
    def forward(
        self, x, return_logits=False, return_head_logits=False, return_all=False
    ):
        ...
        q = (
            self.q(tokens_frame)
            .reshape(B * S, 1, self.num_heads, self.embed_dim)
            .transpose(-3, -2)
        )
        k = (
            self.k(tokens_segment)
            .reshape(B * S, R - 1, self.num_heads, self.embed_dim)
            .transpose(-3, -2)
        )
