...
class RewardPredictor(nn.Module):
    def __init__(
        self, encoder, action_shape, accumulate_steps, hidden_dim, num_filters=32
    ):
        super().__init__()
        ...
        self.mlp = nn.Sequential(
            nn.Linear(
                self.encoder.out_dim + action_shape[0] * accumulate_steps, hidden_dim
            ),
            nn.ReLU(),
...
class InverseDynamicPredictor(nn.Module):
    def __init__(
        ...
        self.mlp = nn.Sequential(
            nn.Linear(
                self.encoder.out_dim * 2 + (accumulate_steps - 1) * action_shape[0],
                hidden_dim,
            ),
            nn.ReLU(),
            nn.Linear(hidden_dim, hidden_dim),
