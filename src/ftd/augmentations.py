...
import ftd.src.ftd.utils as utils
...
def random_crop(x, size=84, w1=None, h1=None, return_w1_h1=False):
    """
    A CUDA-optimized function that performs a random crop operation on images.
    The images are cropped to the specified size at a random location.
    Args:
        x (torch.Tensor): Input images of shape (B, C, H, W) where B is the batch size,
                          C is the number of channels, and H and W are the height and width
                          of the images respectively.
        size (int, optional): The size of the cropped images. Defaults to 84.
        w1 (torch.LongTensor, optional): The x-coordinate of the top-left corner of the crop box.
                                         If not specified, a random value is used.
        h1 (torch.LongTensor, optional): The y-coordinate of the top-left corner of the crop box.
                                         If not specified, a random value is used.
        return_w1_h1 (bool, optional): Whether to return the coordinates of the crop box.
                                       Defaults to False.
    Returns:
        cropped (torch.Tensor): The cropped images of shape (B, C, size, size).
        w1, h1 (torch.LongTensor, optional): The x and y coordinates of the top-left corner
                                             of the crop box, if return_w1_h1 is True.
    """
    # Ensure the specified parameters are correct.
    ...
    img_size = x.shape[-1]  # Image size
    crop_max = img_size - size  # Maximum possible crop size

    # If the crop size is larger than the image, return the original image.
    if crop_max <= 0:
        if return_w1_h1:
            return x, None, None
        return x

    # Permute dimensions to (B, H, W, C) for cropping
    x = x.permute(0, 2, 3, 1)

    # If the top-left corner of the crop box is not specified, randomly generate it.
    if w1 is None:
        w1 = torch.LongTensor(n).random_(0, crop_max)
        h1 = torch.LongTensor(n).random_(0, crop_max)

    # Create a sliding window view of the image for easy cropping
    windows = view_as_windows_cuda(x, (1, size, size, 1))[..., 0, :, :, 0]
    # Perform the crop operation
    cropped = windows[torch.arange(n), w1, h1]

    # Return the cropped images and optionally the crop box coordinates
    if return_w1_h1:
