...
def write_info(args, fp):
    data = {"timestamp": str(datetime.now()), "args": vars(args)}
    with open(fp, "w") as f:
...
def transfer_to_ann(obs, logits, channels, region_num, stack_num, text_height=12):
    ...
    assert (
        len(logits) == 9 or len(logits) == 16
    ), "Currently only support 9 or 16 subfigures."
    ...
    for index in range(region_num):
        ...
        new_image = Image.new(
            "RGB",
            (pil_image.width + text_height, pil_image.height + text_height),
            (255, 255, 255),
        )
        new_image.paste(pil_image, (text_height // 2, 0))
        draw = ImageDraw.Draw(new_image)
        draw.multiline_text(
            (29 + text_height / 2, pil_image.height),
            logit,
            fill=(65, 105, 225),
            font=font,
        )
        total_images.paste(
            new_image, (index // side_num * image_size, index % side_num * image_size)
        )

    return np.array(total_images)


def transfer_to_segments(
    obs, channels, region_num, stack_num, plot_num, blank_height=4
):
    obs = np.array(obs)
    if channels * region_num * stack_num == obs.shape[0]:
        obs = obs[-channels * region_num :]
    ...
    for index in range(plot_num):
        ...
        new_image = Image.new(
            "RGB",
            (pil_image.width + blank_height, pil_image.height + blank_height),
            (255, 255, 255),
        )
...
def show_gpu(simlpe=False):
    ...
    print(
        "Name: [{}], GPU num: [{}], Total mem: [{}G], Free mem: [{}G], Used mem: [{}G], Usage: [{}%]".format(
            gpu_name,
            gpu_num,
            total_memory,
            total_free,
...
def cosine_distance(x, y):
    ...
    return torch.atan2(_sqrt(1.0 - cos_sim**2), cos_sim)
...
def representation_distances(
    ...
    first_squared_reps = torch.reshape(
...
def target_distances(representations, reward_diffs, cumulative_gamma):
    next_state_distances = representation_distances(representations, representations)
...
def reward_distance(rewards):
    ...
    squared_rews_transp = torch.reshape(
        squared_rews_transp, (squared_rews_transp.shape[0] ** 2, 1)
    )
...
def squarify(x):
    ...
    if len(x.size()) > 1:
        ...
        return torch.reshape(
            x.repeat(1, batch_size), (batch_size, batch_size, repre_dim)
        )
