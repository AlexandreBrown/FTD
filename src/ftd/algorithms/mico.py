...
class MICo(object):
    ...
    def update_encoder(self, obs, reward, next_obs, L, step):
        ...
        torch.nn.utils.clip_grad_norm_(
            parameters=self.critic.encoder.parameters(), max_norm=self.max_norm
        )
        self.encoder_optimizer.step()

    def update_critic(
        self, obs, action, reward, next_obs, not_done, L, step, frozen=False
    ):
        with torch.no_grad():
    ...
    def update(
        ...
        self.update_critic(
            obs, action, reward, next_obs, not_done, L, step, frozen=frozen
        )
