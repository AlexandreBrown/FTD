...
class Q2(object):
    ...
    def __init__(
        ...
        actor_encoder = m.Encoder(shared_cnn, head_cnn, actor_projection)
        critic_encoder = m.Encoder(shared_cnn, head_cnn, critic_projection)
        ...
        self.dsa_critic = m.make_siamese_net(
            "critic",
            args.projection_dim,
            action_shape[0],
            [1024, 32],
        ...
        self.dsa_actor = m.make_siamese_net(
            "actor", args.projection_dim, action_shape[0], [1024], max_action=1.0
        ).cuda()
    ...
    def update_encoder(self, obs, action, reward, next_obs, L, step):
        ...
        with torch.no_grad():
            ...
            next_h_target_sq = torch.reshape(
                next_h_target_sq, (batch_size**2, self.projection_dim)
            )
        ...
        if self.num_dsa % self.dsa_act_update_freq == 0:
            ...
            h_target_sq = h_target_sq.permute(1, 0, 2)
            h_sq = torch.reshape(h_sq, (batch_size**2, self.projection_dim))
            h_target_sq = torch.reshape(
                h_target_sq, (batch_size**2, self.projection_dim)
            )
        ...
        if step % self.critic_target_update_freq == 0:
            ...
            if step % self.dsa_target_update_freq == 0:
                for param, target_param in zip(
                    self.dsa_critic.parameters(), self.dsa_critic_target.parameters()
                ):
                    target_param.data.copy_(
                        self.sia_tau * param.data
                        + (1 - self.sia_tau) * target_param.data
                    )
