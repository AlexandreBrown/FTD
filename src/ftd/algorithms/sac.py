...
class SAC(object):
    ...
    def update_inverse_dynamic_predictor(self, obs, action, next_obs, L, step):
        predicted_action = self.inverse_dynamic_predictor(
            obs, next_obs, torch.tensor([]).cuda()
        )
    ...
    def update_forward_dynamic_predictor(self, obs, action, next_obs, L, step):
        ...
        predict_loss = self.forward_factor * F.mse_loss(
            predicted_next_obs, embeded_next_obs
        )
        ...
        if self.max_grad_norm != 0:
            torch.nn.utils.clip_grad_norm_(
                self.forward_dynamic_predictor.parameters(), self.max_grad_norm
            )
