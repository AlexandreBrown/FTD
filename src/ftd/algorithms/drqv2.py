...
class DrQV2Agent:
    def __init__(self, obs_shape, action_shape, args):
        ...
        self.actor = Actor(
            self.encoder.repr_dim, action_shape, args.projection_dim, args.hidden_dim
        ).cuda()
        ...
        self.encoder_opt = torch.optim.Adam(
            self.encoder.parameters(), lr=args.selector_lr
        )
    ...
    def update(self, replay_buffer, L, step):
        obs, action, reward, discount, next_obs = replay_buffer.sample_drqv2(
            n_step=self.n_step, discount=self.discount
        )
